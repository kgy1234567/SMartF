...
module.exports = {
    ...
    getCurrentSensor: function(callback) {
        ...
        db.each(sql, function(err, row) {
            if (err) {
                console.error('getCurrentSensor 오류', err);
                return;
            }
            callback(row);
        });
    ...
    getCurrentActuator: function(callback) {
        ...
        db.each(sql, function(err, row) {
            if (err) {
                console.error('getCurrentActuator 오류', err);
                return;
            }
            callback(row);
        });
    ...
    insertActuator: function(redLED,relay,reason,uid,greenLED,blueLED, callback) {
        let db = new sqlite3.Database("db/smartfarm.db");
        let sql = `insert into actuator (redLED,relay,reason,uid,greenLED,blueLED) values (?,?,?,?,?,?)`;
        let stmt = db.prepare(sql);
        stmt.run(redLED,relay,reason,uid,greenLED,blueLED, function(err) {
            if (err) {
